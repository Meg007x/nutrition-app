  ...
  for (const meal of meals) {
    ...
    const id = await Notifications.scheduleNotificationAsync({
      content: {
        ...
        sound: "default",
        data: { screen: "/scan" }, // 👈 เพิ่มบรรทัดนี้
      },
      trigger: {
...
export async function scheduleWaterReminder() {
  if (isWeb) return { ok: false, reason: "web_not_supported" as const };
  ...
  await Notifications.scheduleNotificationAsync({
    content: {
      ...
      body: "พักดื่มน้ำสักแก้ว เพื่อสุขภาพที่ดีครับ",
      sound: "default",
      data: { screen: "/water" }, // 👈 ให้วิ่งไปหน้าบันทึกน้ำ
    },
    trigger: {
      type: Notifications.SchedulableTriggerInputTypes.TIME_INTERVAL,
      seconds: 3600, // 3600 วินาที = 1 ชั่วโมง
      repeats: true,
    },
  });
